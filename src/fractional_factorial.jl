...
function generate_ff_sample_matrix(design_matrix::Array{Int64,2},  levels_list=nothing)
      ...
      """
      sample_matrix = zeros(size(design_matrix)[1], size(design_matrix)[2])
      if levels_list == nothing
           sample_matrix[design_matrix.==-1] .= 0
           sample_matrix[design_matrix.==1] .= 1
      else
           @assert length(levels_list) == size(design_matrix)[2] "The length of the levels list must be equal to the number of parameters"
...
function ff_main_effects(design_matrix::AbstractArray, response_values::AbstractArray)
      ...
      for column in 1:size(design_matrix)[2]
            contrast_vector = @view design_matrix[:,column]
            main_effects[column] = dot(contrast_vector, response_values)/(length(contrast_vector))
      end
      return main_effects
end

function ff_variance(design_matrix::AbstractArray, response_values::AbstractArray)
      """
           For each parameter, compute the variance of the results explained by that parameter.
           ...
           See Saltelli, A. equation 2.33 for details.
      """

      return ff_main_effects(design_matrix, response_values).^2
end
