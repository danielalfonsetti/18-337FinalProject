...
for (index, sample_size) in enumerate(sample_sizes)
        X, Y = create_ishigami_samples(sample_size)
        res = delta_moment_analyze_mutlithreaded(X[:,4], Y, Ygrid_length=110, num_classes=10)
...
scatter!(
        ...
        label="Bias Corrected Delta Index",
        xaxis=:log,
        xticks = (sample_sizes, sample_sizes),
        legend=:topright,
)
savefig("./18337FinalProject/plots/delta_ishigami_function_inactive_variable.png")
...
reptitions = 4
delta_reptitions = zeros(reptitions, 4)
for reptition in 1:reptitions
        sample_size = 512
        X, Y = create_ishigami_samples(sample_size)
        delta, delta_adjusted, delta_low, delta_high = delta_moment_analyze(X, Y, Ygrid_length=110, num_classes=10)
        delta_reptitions[reptition,:] = delta
end

mean(delta_reptitions, dims=1)
mean(delta_reptitions, dims=1)
...
sample_size = 2^14
X, Y = create_ishigami_samples(sample_size)
# delta_moment_analyze_mutlithreaded(sample_size)
delta_moment_analyze_mutlithreaded(X, Y; num_resamples=10)


using Profile
@profile for i in 1:10 delta_moment_analyze(X, Y, Ygrid_length=110, num_classes=10, num_resamples=10) end
Juno.profiler()
Profile.clear()
